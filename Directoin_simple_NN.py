...
x_example=mean_movement(data_training,[4])[0]
y_example=mean_movement(data_training,[4])[1]

x_train,x_val = np.split(np.array(x_example),2)
y_train,y_val = np.split(np.array(y_example),2)

print(x_val[0:10],y_val[0:10])
x_test=mean_movement(data_testing,[4])[0]
y_test=mean_movement(data_testing,[4])[1]
draw_test=mean_movement(data_testing,[4])[2]
...
outputs = tf.keras.layers.Dense(1,activation='tanh', name="predictions")(x)
...
model.save('Neural_Networks/mean_direction')
