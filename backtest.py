...
import json
from strats import *
from harmonic_strategie import *
...
class BACKTEST_v3():
    def __init__(self): #Innitializing all the required variables
        ...
        self.long_text="긴"
        self.short_text='짧은'
        self.column_names=['Hash','StartId','CloseId', 'Symbol', 'Type', 'BuyPrice', 'SellPrice', 'Profit', 'Note']

    def tick(self): #tick reffers to receiving a new candle from the market
        self.index+=1

    def MakeOrder(self,symbol,o_type,note):
        ...
        self.open_orders[hax] = [hax, symbol, o_type, buy_price, exe_id, note]
